...
| _nonCreditLineBalances | mapping(address => uint256) | Maps non-credit line addresses to their token balances.     |

## Errors

### UnexpectedTokenSource

```solidity
error UnexpectedTokenSource();
```
Thrown when an unexpected token source is encountered during a withdrawal operation.

### OutOfTokenBalance
```solidity
error OutOfTokenBalance();
```
Thrown when the token balance is insufficient to fulfill a withdrawal request.
...
```
The constructor initializes the contract with the address of the associated lending market and the lender. It performs basic input validation for these addresses.
