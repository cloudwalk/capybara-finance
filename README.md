...
4. **Lending Market:** The Lending Market is the primary interface for borrowing and repaying loans. It is also responsible for tracking and enforcing the states and statuses of all loans.
5. **Lending Registry:** The Lending Registry allows for the creation of new credit lines and liquidity pools.
6. **CapybaraNFT** The non-fungible token contract, where each minted token represents the loan with the corresponding id.
7. **Liquidity Pool Factory:** A smart contract to create and manage different kinds of liquidity pools within the protocol. Currently, it supports creating only a specific kind of accountable liquidity pool but is designed to potentially support more kinds in the future.
8. **Credit Line Factory:** A smart contract to create and manage different kinds of credit lines within the protocol. Currently, it supports creating only a specific kind of configurable credit line but is designed to potentially support more kinds in the future.
...
## [CapybaraNFT](./documentation/CapybaraNFT.md)

`CapybaraNFT` is an innovative feature of the CapybaraFinance protocol, where each NFT represents a unique loan within the ecosystem. It offers a creative means of symbolizing financial transactions as collectible and tradeable assets on the blockchain.

### Key Features of CapybaraNFT:

- Loan Representation as NFT: Each loan, when issued, is represented as a unique NFT, creating a tangible asset out of a financial agreement.
- Ownership Tracking: The ownership of the NFT correlates with the parties involved in the loan, offering a visual and proprietary track of the loan's lifecycle.
- Interoperability: `CapybaraNFT` is compatible with the broader NFT market, allowing for the possibility of trading loans as NFTs within various marketplaces.
- Enhanced Liquidity: By converting loans into NFTs, it may introduce a new level of liquidity for loans as they can be bought, sold, or traded like other NFTs.

### Implementation Details:

The NFT contract is compliant with the ERC-721 standard, ensuring it works seamlessly with existing infrastructure and wallets that support NFTs.
`CapybaraNFT` integrates with the Lending Market and Credit Line to automatically mint and manage NFTs as loans are issued and repaid.

###### Interaction with Loans and Borrowers:
The process of minting an NFT is triggered by the initiation of a loan in the lending market.
Borrowers can potentially use their `CapybaraNFT` as collateral in other decentralized finance (DeFi) applications.
The transfer of an NFT can be restricted to enforce the adherence to the underlying loan terms and compliance with the regulatory requirements.
The addition of `CapybaraNFT` to the CapybaraFinance protocol blends traditional finance with the latest in blockchain technology, offering novel avenues for investment, trading, and finance management.
