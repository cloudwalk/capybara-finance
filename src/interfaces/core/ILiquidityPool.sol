...
/// @notice Defines the liquidity pool functions and events
...
interface ILiquidityPool {
    ...
    function onBeforeLoanTaken(uint256 loanId, address creditLine) external;

    /// @notice A hook that is triggered by the market after a loan is taken
    /// @param loanId The unique identifier of the loan being taken
    /// @param creditLine The address of the associated credit line
    function onAfterLoanTaken(uint256 loanId, address creditLine) external;
    ...
    function onBeforeLoanPayment(uint256 loanId, uint256 repayAmount) external;
