...
library InterestMath {
    ...
    /*
    * Minimum value that signed 64.64 bit fixed point number may contain.
    */
    int128 private constant MIN_64x64_VALUE = -0x80000000000000000000000000000000;

    /*
     * Maximum value that signed 64.64 bit fixed point number may contain.
     */
    ...
    /// @notice Thrown when the calculation results overflows or underflows the bounds of the used type
    ...
    function calculateOutstandingBalance(
        ...
        if (interestFormula == Interest.Formula.Compound) {
            return _calculateOutstandingBalanceRounded(originalBalance, numberOfPeriods, interestRate, interestRateFactor);
        } else {
    ...
    /**
    * @dev Calculates the result of dividing unsigned 256-bit integer `numerator` by non-zero unsigned 256-bit integer `denominator`,
    *      with rounding towards zero.
    * @dev Reverts if `denominator` is zero or if the operation overflows the 64.64-bit fixed-point range.
    *
    ...
    */
    ...
    /**
    * @dev Calculates the result of raising a signed 64.64 fixed-point number to the power of an unsigned 256-bit integer.
    * @dev Reverts in case of arithmetic overflow.
    *
    * @param base The signed 64.64 fixed-point base number.
    * @param exponent The unsigned 256-bit integer exponent.
    *
    ...
    */
    function power(int128 base, uint256 exponent) internal pure returns (int128) {
        unchecked {
            ...
            if (absoluteBase <= 0x10000000000000000) {
                ...
                if (absoluteBase < 0x1000000000000000000000000) { absoluteBase <<= 32; absoluteBaseShift -= 32; }
                if (absoluteBase < 0x10000000000000000000000000000) { absoluteBase <<= 16; absoluteBaseShift -= 16; }
                if (absoluteBase < 0x1000000000000000000000000000000) { absoluteBase <<= 8; absoluteBaseShift -= 8; }
                if (absoluteBase < 0x10000000000000000000000000000000) { absoluteBase <<= 4; absoluteBaseShift -= 4; }
                if (absoluteBase < 0x40000000000000000000000000000000) { absoluteBase <<= 2; absoluteBaseShift -= 2; }
                if (absoluteBase < 0x80000000000000000000000000000000) { absoluteBase <<= 1; absoluteBaseShift -= 1; }
    ...
    /**
    * @dev Calculates the result of multiplying signed 64.64 fixed-point numbers `multiplicanda` and `multiplier`,
    *      with rounding down.
    * @dev Reverts in case of arithmetic overflow.
    *
    * @param multiplicanda The first signed 64.64-bit fixed-point number.
    ...
    */
    function multiply(int128 multiplicanda, int128 multiplier) internal pure returns (int128) {
        unchecked {
            int256 product = int256(multiplicanda) * multiplier >> 64;
    ...
    function _divide(uint256 numerator, uint256 denominator) internal pure returns (uint128) {
        unchecked {
            ...
            if (numerator <= 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF)
                ...
                if (numeratorShifted >= 0x100000000) { numeratorShifted >>= 32; msb += 32; }
                if (numeratorShifted >= 0x10000) { numeratorShifted >>= 16; msb += 16; }
                if (numeratorShifted >= 0x100) { numeratorShifted >>= 8; msb += 8; }
                if (numeratorShifted >= 0x10) { numeratorShifted >>= 4; msb += 4; }
                if (numeratorShifted >= 0x4) { numeratorShifted >>= 2; msb += 2; }
                if (numeratorShifted >= 0x2) msb += 1;
                ...
                if (numeratorLow < low) numeratorHigh -= 1;
                numeratorLow -= low;
                low = high << 128;
                if (numeratorLow < low) numeratorHigh -= 1;
                numeratorLow -= low;
